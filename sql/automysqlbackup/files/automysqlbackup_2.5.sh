...
# DBNAMES="mysql pureftpd "
DBNAMES=$(mysql --defaults-file=/etc/mysql/debian.cnf -e "show databases;" | grep -v "Database" | grep -v "information_schema" | grep -v "performance_schema" | sed 's/[|+-]+//g' | xargs echo)


# Backup directory location e.g /backups
BACKUPDIR="/srv/mysql_backup"
...
MAILADDR="exploitation.alertes@brainsonic.com"
...
BACKUPFILES=""
# --master-data
OPT="--routines --quote-names --opt --max_allowed_packet=512M "	# OPT string for use with mysqldump (man mysqldump )
mysqldump --help |grep -qws events && OPT="--routines --quote-names --opt --max_allowed_packet=512M --events --ignore-table=mysql.events"			# OPT string for use with mysqldump (man mysqldump )
...
mysqldump --defaults-file=/etc/mysql/debian.cnf --host=$DBHOST $OPT $1 > $2
return 0
}
...
if [ "$SEPDIR" = "yes" ]; then # Check if CREATE DATABSE should be included in Dump
	if [ "$CREATE_DATABASE" = "no" ]; then
		...
		OPT="$OPT --databases --lock-tables=false --skip-add-locks --single-transaction --quick"
	fi
else
	OPT="$OPT --databases"
fi
...
if [ "$DBNAMES" = "all" ]; then
        DBNAMES="`mysql --defaults-file=/etc/mysql/debian.cnf --host=$DBHOST --batch --skip-column-names -e "show databases"| sed 's/ /%/g'`"
